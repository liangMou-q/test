...
<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.goodsName" clearable placeholder="商品名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="入库日期" width="160px" align="center">
        <template slot-scope="{row}">
          <!-- <span>{{ row.createdAt | parseTime('{y}-{m}-{d} {h}:{i}') }}</span> -->
          <span>{{ parseTime(row.inData) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="入库编号" width="160px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.inCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="进货人" width="100px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="入库价格" width="110px">
        <template slot-scope="{row}">
          <!-- <svg-icon v-for="n in + row.importance" :key="n" icon-class="star" class="meta-item__icon" /> -->
          <span>{{ row.inPrice }}</span>
        </template>
      </el-table-column>
      <el-table-column label="供应商编码" width="160px">
        <template slot-scope="{row}">
          <!-- <svg-icon v-for="n in + row.importance" :key="n" icon-class="star" class="meta-item__icon" /> -->
          <span>{{ row.supCode }}</span>
        </template>
      </el-table-column>
...
import { stockList, insertstock } from '@/api/stock'
...
export default {
  ...
  data() {
    return {
      ...
      searchForm: [
        ...
        { type: 'input', label: '入库价格', prop: 'inPrice', placeholder: '请输入日期' },
        { type: 'input', label: '进货人:', prop: 'purchaser', placeholder: '请编辑内容' },
        { type: 'input', label: '供应商:', prop: 'supCode', placeholder: '请编辑内容' }
      ],
      tableKey: 2,
      // // 数据列表
      // list: null,
  ...
  methods: {
     ...
     */
    getStocklist() {
      this.getList(stockList)
    },
    // async getStocklist() {
    //   this.listLoading = true
    //   const { data } = await stockList(this.listQuery)
    ...
    async submit(value) {
      if (this.dialogStatus === 'create' && value) {
        // 新增
        const formData = Object.assign({ inCode: getOnlycode('IN'), inData: new Date() }, { ...value })
        console.log(formData)
        const flag = await this.getLatestmsg(formData.inCode, formData.goodsCode, undefined, Number(formData.inQuantity))
        if (flag) {
          await this.add(insertstock, formData)
        }
    ...
    //  */
    // async add(formData) {
    //   const res = await insertstock(formData)
    //   console.log(res)
    //   this.dialogFormVisible = false
    //   this.$notify({
    //     title: 'Success',
